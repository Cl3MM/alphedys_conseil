...
class JoomSEF
{
                ...
                default: {
                    ...
                    // own extension block
                        ...
                        if (JoomSEF::_isHomePage($uri2, true)) {
                            $title = array();
                            $uri = JoomSEF::_sefGetLocation($uri2, $title, null, null, null, $uri->getVar('lang'));
                            JoomSEF::_endSef($prevLang);
                            return;
                        }
    ...
    function _determineLanguage($getLang = null, $redir = false)
    {
        ...
        if (SEFTools::JoomFishInstalled()) {
            ...
            if (!$jfrouterEnabled && $redir && (isset($cookieCode) || isset($browserCode)) && ($sefConfig->mainLanguage != '0')) {
                if (isset($cookieCode)) {
            ...
            if (empty($lang) && !$jfrouterEnabled) {
                ...
                if (empty($code) || !JLanguage::exists($code)) {
                    ...
                    else if( ($sefConfig->mainLanguage != '0') ) {
    ...
    function _parseSefUrl(&$uri, &$oldUri)
    {
        ...
        if (empty($route)) {
            ...
            JoomSEF::_determineLanguage(JRequest::getVar('lang'), $redir);
            ...
            if ($db->loadResult()) {
                ...
                $query = "SELECT * FROM #__sefurls WHERE sefurl = '' OR sefurl = 'index.php'";
    ...
    function _saveNewURL($Itemid, &$metadata, $priority, $temploc, $url)
    {
        ...
        "VALUES ('".$temploc."', '" . mysql_escape_string(html_entity_decode(urldecode($url)))."', '$priority'" . $val . $metavals . ", " . $traceinfo . ", '{$sm_indexed}', '{$sm_date}', '{$sm_frequency}', '{$sm_priority}')";
        $db->setQuery($query);
